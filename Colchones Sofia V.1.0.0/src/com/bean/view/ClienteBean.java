...
public class ClienteBean implements Serializable {
	...
	private DetalleCompraVenta seleccionar;
	...
	public void initCliente() {
		...
		this.usuario.getRolBean().setRol("CLIENTE");
	}
	...
	///////////////////////////////////////////////////////
	/**
	 * Metodo que permite conocer un cliente por su indice en la tabla.
	 */
	public void cliente() {
		this.message = null;
		this.error = true;
		this.index = -1;
		String documento = Face.get("documento-cliente");
		String tipo_documento = Face.get("tipo-documento-cliente");
		if (Convertidor.isCadena(documento) && Convertidor.isCadena(tipo_documento)) {
			this.index = index(documento, tipo_documento);
			if (this.index >= 0 && this.table.getClientes() != null && this.index < this.table.getClientes().size()) {
				this.usuario = cliente(this.table.getClientes().get(this.index));
				...
				message = new FacesMessage(FacesMessage.SEVERITY_WARN, "", "El cliente no se encuentra registrado.");
			}
		} else {
			message = new FacesMessage(FacesMessage.SEVERITY_WARN, "", "No se ha seleccionado ningun cliente.");
		}
	}
	 ...
	 * @return representa el indice del cliente en la lista.
	 */
	public int index(String documento, String tipo) {
		if (Convertidor.isCadena(documento) && Convertidor.isCadena(tipo)) {
		int aux = 0;
		for (Usuario u : this.table.getClientes()) {
			if (u.getPersona().getDocumento().equals(documento)) {
				if (u.getPersona().getTipoDocumentoBean().getTipoDocumento().equals(tipo)) {
					return aux;
				}
			}
			aux++;
			...
			if (aux == null) {
				...
				if (this.usuario.getPersona() != null) {
					if (this.usuario.getPersona().getTipoDocumentoBean().getTipoDocumento().equals(tipo_documento)) {
						...
						this.validar = cliente(this.usuario);
					} else {
						message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "",
				...
				message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "",
						"Ya esta registrado un cliente con ese documento " + documento + ".");
			}
		} else {
	 ...
	 * @return
	 */
	public Usuario cliente(Usuario v) {
		Usuario u = null;
	...
	public void registrar() {
		this.message = faceRegistrarCliente();
		if (this.message != null) {
	...
	public void actualizar() {
		this.message = faceActualizarCliente();
		if (this.message != null) {
			FacesContext.getCurrentInstance().addMessage(null, this.message);
		}
	}
	...
	public void eliminar() {
		this.message = faceEliminarCliente();
		if (this.message != null) {
			FacesContext.getCurrentInstance().addMessage(null, this.message);
		}
	}
	...
	/**
	 * Metodo que cambia el valor del estado del cliente.
	 */
	public void estado() {
		this.update = false;
		String documento = Face.get("documento-cliente");
		if(Convertidor.isCadena(documento)) {
			...
			if(aux != null) {
				...
				dao.update(aux);
				this.table.initCliente();
				this.update = true;
				this.message = new FacesMessage(FacesMessage.SEVERITY_INFO, "",
						"Se ha cambiado el estado al cliente con documento " + aux.getPersona().getDocumento()
	...
	///////////////////////////////////////////////////////
	/**
	 * Metodo que permite validar los campos del cliente.
	 * 
	 * @param v representa el cliente.
...
///////////////////////////////////////////////////////
	/**
	 * Metodo que obtiene el estato del cliente a registrar.
	 */
	...
	/**
	 * Metodo que obtiene el estato del cliente a actualizar.
	 */
	...
	///////////////////////////////////////////////////////
	/**
	 * Metodo que permite insertar el cliente.
	 * 
	 * @return representa el mensaje obtenido.
	 */
	@SuppressWarnings({ "deprecation" })
	private FacesMessage faceRegistrarCliente() {
		...
		if (message == null) {
			...
			if (p != null && !this.error) {
				...
				if (aux == null) {
					...
					this.table.setRenderizar_clientes(0);
					this.table.initCliente();
					this.initCliente();
				} else {
				...
				this.message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "", "No se ha registrado el cliente.");
			}
		}
		return message;
	}
	 ...
	 */
	private FacesMessage faceActualizarCliente() {
		this.message = validar(this.usuario, sesion.getLogeado());
		this.update = false; 
		if (!this.error) {
			this.error = true;
	 ...
	 * @return representa el mensage generado.
	 */
	private FacesMessage faceEliminarCliente() {
		this.remove = false;
		String documento = Face.get("documento-cliente");
		if (Convertidor.isCadena(documento)) {
			UsuarioController dao = new UsuarioController();
	...
	public DetalleCompraVenta getSeleccionar() {
		return seleccionar;
	}

	public void setSeleccionar(DetalleCompraVenta seleccionar) {
